...
### Enhanced Streamlit Dashboard### --day17
...
```
# ##Streamlit Application### --day19

## Overview

A Streamlit-based web application was developed to provide an interactive interface for mushroom yield prediction. The application allows users to enter environmental sensor readings and receive an estimated mushroom yield generated by the deployed machine learning model.

The deployed model is the champion model selected during model evaluation and comparison.
...
The application accepts the following sensor readings through an interactive sidebar:

- Temperature (°C)
- Humidity (%)
- CO₂ Concentration (ppm)

Users can adjust values using sliders and submit inputs for prediction.

### Yield Prediction

The application predicts expected mushroom yield in kilograms using the trained Random Forest model and displays the result using Streamlit metrics.

### Input Validation

Warning messages are displayed when sensor readings fall outside the ranges observed during model training. This helps users understand when predictions may be less reliable.

### Sensitivity Analysis

A humidity sensitivity chart is included to demonstrate how predicted yield changes as humidity varies while temperature and CO₂ remain fixed.

This feature supports simple what-if analysis for operational decision making.
